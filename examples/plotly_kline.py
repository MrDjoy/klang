...
def generate_portfolio_tracking(symbols=None):
    ...
    for i in range(len(dates)):
        ...
        benchmark_value = initial_value * (
                    1 + (avg_ytd_change / 100) * 0.85 * progress)  # Benchmark slightly underperforms portfolio
...
def generate_stock_analysis(symbol='TSLA'):
    ...
    try:
        ...
        colors = [COLORS['positive'] if row['Close'] >= row['Open'] else COLORS['negative'] for i, row in
                  hist.iterrows()]
        ...
        change_1wk = ((hist['Close'].iloc[-1] - hist['Close'].iloc[-5]) / hist['Close'].iloc[-5]) * 100 if len(
            hist) >= 5 else 0
        change_1mo = ((hist['Close'].iloc[-1] - hist['Close'].iloc[-21]) / hist['Close'].iloc[-21]) * 100 if len(
            hist) >= 21 else 0
        change_3mo = ((hist['Close'].iloc[-1] - hist['Close'].iloc[-63]) / hist['Close'].iloc[-63]) * 100 if len(
            hist) >= 63 else 0
